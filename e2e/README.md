...
- Rendering Gemini recommendation fixtures for personalized and mood flows
...
The Playwright config sets `MEDIA_DB_PATH=data/e2e-test.sqlite`, so e2e runs use an isolated SQLite database and do not touch `data/media.sqlite`. It also blanks Gemini and TMDB API credentials for the test server, so the suite does not require external API keys. TMDB search e2e coverage runs with `FRAMELOG_E2E_TMDB_FIXTURES=true`, which makes the backend return deterministic fixture results instead of calling the real TMDB API. Gemini recommendation e2e coverage runs with `FRAMELOG_E2E_GEMINI_FIXTURES=true`, which makes the backend return deterministic recommendation fixtures instead of calling the real Gemini API. `e2e/global-setup.js` deletes the test database plus its WAL and SHM files before each run; npm also runs that cleanup before `npm run dev`, so the database is removed before the Express process opens it.
